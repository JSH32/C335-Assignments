...
int get_opcode(int instruction) {
  /*
  Weird method of encoding 🤷, why not just do like 0x33 for arithematic?
  Threw me off for a sec since I was familiar with the RISC-V spec and those
  opcodes didn't exist and I thought it was made up for a sec.
  I actually evaluated the bitwise and it makes sense. Is this an anti-GPT measure?
  */
...
int get_destination(int instruction) {
  // Destination register (7-11 bits).
  return (instruction >> 7) & 0x1F;
}

int get_load_address(int instruction) {
  // Source register for load (15-19 bits).
  return (instruction >> 15) & 0x1F;
}

int get_load_offset(int instruction) {
  // Immediate offset for load (20-31 bits).
  return (instruction >> 20) & 0xFFF;
}

int get_store_offset(int instruction) {
  // Combine lower (7-11 bits) and upper (25-31 bits).
...
int get_store_source(int instruction) {
  // Source register for store (20-24 bits).
  return (instruction >> 20) & 0x1F;
}

int get_store_address(int instruction) {
  // Base address register for store (15-19 bits).
  return (instruction >> 15) & 0x1F;
}
...
int get_math_operand_a(int instruction) {
  // First operand register (15-19 bits).
  return (instruction >> 15) & 0x1F;
}

int get_math_operand_b(int instruction) {
  // Second operand register (20-24 bits).
  return (instruction >> 20) & 0x1F;
}

int get_constant_value(int instruction) {
  // Constant value (12-31 bits).
